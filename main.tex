...
\usepackage{nameref}  % reference unnumbered sections by name
\usepackage[sort&compress,numbers,super]{natbib}  % superscript citations
\AtBeginDocument{\nocite{achemso-control}}  % read @Control in .bib file
...
\noindent{\bf Keywords:} quantum electrodynamics, density functional theory, 2D
materials, transmission electron microscopy, electronic excitation, sputtering,
defect engineering
\pagebreak
...
% $P_i$.
The next section titled \textit{\nameref{sec:probability}} describes how to
combine QED with DFT to obtain $P_i$.
% The next section describes how to combine QED with DFT to obtain $P_i$.
...
The following section titled \textit{\nameref{sec:sputtering}} considers the
evolution of the excited states during the sputtering process to derive
$\{E_d\}$ and $P_f$.
It then demonstrates how our formalism significantly improves the prediction of
sputtering rates in hBN and MoS$_2$.
...
occupied and unoccupied states (see \textit{\nameref{sec:crystal}}).
...
\medskip\noindent
% \textit{Assumption 3: the PKA intially travels at a constant velocity}.
...
given in the subsections titled \textit{\nameref{sec:hBN}} and
\textit{\nameref{sec:MoS2}}.
...
% \ref{sec:assumptions}.
$n_i^\text{max}$ for a given material, as described at the end of 
...
\subsection*{Boron and nitrogen sputtering from hBN}
\label{sec:hBN}
%------------------------------------------------------------------------------
...
Fitting our cross section curves to the data of Cretu \textit{et al.} yields an excellent
...
\subsection*{Sulfur sputtering from MoS$_2$}
\label{sec:MoS2}
%------------------------------------------------------------------------------
...
with a $\Gamma$-centered $6\times6\times1$ Monkhorst-Pack
meshes, \cite{Monkhorst1976} corresponding to 0.417 and 0.328 and k-points per
inverse {\AA} for hBN and MoS$_2$ respectively.
...
% Suppose that the transmitted photon behaves like a free particle, \textit{i.e.}, it
...
% morphology of an irradiated material with sub-angstrom precision \textit{via} selective
